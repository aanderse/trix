...
let
  ...
  resolveFollowsWithVisited = visited: path:
    let
      ...
      step = nodeName: elem:
        ...
        in
          if builtins.isString ref then ref
            ...
            let refKey = builtins.concatStringsSep "/" ref;
            in if builtins.elem refKey newVisited
               then throw "trix: circular follows detected: ${builtins.concatStringsSep " -> " (newVisited ++ [refKey])}"
               else resolveFollowsWithVisited newVisited ref;
  ...
  fetchSource = name: node: basePath:
    let
    ...
    in
      if type == "github" then
        ...
        } // (if locked ? ref then { ref = locked.ref; } else {}))
    ...
    in
      ...
      # For flake inputs, import their flake.nix and call outputs
        ...
        let
          ...
          inputFlakeLock =
            if builtins.pathExists (src + "/flake.lock")
            then builtins.fromJSON (builtins.readFile (src + "/flake.lock"))
          ...
          inputFlakeInputNames =
            if inputFlake ? inputs
            then builtins.attrNames inputFlake.inputs
            else builtins.attrNames (builtins.functionArgs inputFlake.outputs);

          # Our overrides from the main lock file (follows, etc.)
          nodeInputs = node.inputs or {};

          # Build each input the flake needs
          inputInputs = builtins.listToAttrs (map (iname: {
            name = iname;
            value =
              if nodeInputs ? ${iname} then
                # We have an override (follows or direct ref)
                let ref = nodeInputs.${iname};
                in if builtins.isString ref
                   then buildInput iname nodes.${ref} flakeDirPath
                   else resolveFollows ref
  ...
  lockedInputs = builtins.mapAttrs (name: ref:
