...
/// Pin an existing registry entry to a specific revision.
pub fn pin_registry_entry(name: &str, rev: &str) -> Result<()> {
    let mut user_registry = load_registry_file(&get_user_registry_path());

    // Find and update the entry
    for entry in &mut user_registry.flakes {
        if entry.from.from_type == "indirect" && entry.from.id == name {
            entry.to.rev = Some(rev.to_string());
            save_user_registry(&user_registry)?;
            return Ok(());
        }
    }
